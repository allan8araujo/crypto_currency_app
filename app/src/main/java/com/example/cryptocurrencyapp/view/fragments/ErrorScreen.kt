...
class ErrorScreen() : Fragment() {
    ...
    private fun bindDataError(dataResults: DataResult.Error<List<AssetsItem>>) {
        when (dataResults.throwable.code()) {
            ...
            401 ->
                binding.errorTextView.text =
                    "Erro ao carregar lista de moedas. Considere trocar a chave de api."
            403 ->
                binding.errorTextView.text =
                    "Erro ao carregar lista de moedas. Sua chave de api não tem previlégios o suficiente."
            429 ->
                binding.errorTextView.text =
                    "Erro ao carregar lista de moedas. Você excedeu o limite de sua chave."
            550 ->
                binding.errorTextView.text =
                    "Erro ao carregar lista de moedas. Você excedeu os limite de chamdas com essa chave de api."
        }
    }
}
